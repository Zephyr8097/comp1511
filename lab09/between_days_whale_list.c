...
void between_days_whales(struct pod *first_pod, struct date *start_day, struct date *finish_day) {
    ...
    // PUT YOUR CODE HERE
    for(struct pod *curr = first_pod; curr != NULL; curr = curr->next) {
        int compare = (curr->when->year) * 10000 + (curr->when->month) * 100 + (curr->when->day);
        int start = (start_day->year) * 10000 + (start_day->month) * 100 + (start_day->day);
        int finish = (finish_day->year) * 10000 + (finish_day->month) * 100 + (finish_day->day);
        if (compare >= start && compare <= finish) {
            if (curr->when->day < 10 && curr->when->month >= 10) {
                printf ("0%d/%d/%d %d %s\n",curr->when->day, curr->when->month, curr->when->year,
                        curr->how_many, curr->species);
            } else if (curr->when->day >= 10 && curr->when->month < 10) {
                printf ("%d/0%d/%d %d %s\n",curr->when->day, curr->when->month, curr->when->year,
                        curr->how_many, curr->species);
            } else if (curr->when->day < 10 && curr->when->month < 10) {
